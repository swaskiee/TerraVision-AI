...
# Device is set inside train() so DataLoader workers don't init CUDA on import.
device = torch.device("cpu")  # overridden in train()
...
train_transform = transforms.Compose([
    ...
    transforms.RandomErasing(p=0.2, scale=(0.02, 0.15), ratio=(0.4, 2.5)),
])
...
def worker_init_fn(worker_id):
    """Register the 3LC URL alias inside each DataLoader worker process."""
    import logging
    logging.getLogger("tlc").setLevel(logging.ERROR)
    import tlc as _tlc
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        for images, labels in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
            ...
            rand_idx = torch.randperm(images.size(0), device=device)
